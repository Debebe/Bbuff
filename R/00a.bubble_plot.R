...
prog.cst <- prog_costs %>%
  select(
    country, g_whoregion, e_inc_ds_num,
    e_inc_rr_num, gdp, exp_dstb, exp_mdrtb
  )

care.cst <- care_costs %>%
  select(
    country, g_whoregion, c.opd_dstb.m,
    c.opd_mdrtb.m, c.ipd_dstb.m, c.ipd_mdrtb.m
  )


tbcst <- prog.cst %>% left_join(care.cst, by = c("country", "g_whoregion"))
tb <- c(
  "exp_dstb", "exp_mdrtb", "cdstb.opd",
  "cmdrtb.opd", "cdstb.ipd", "cmdrtb.ipd"
)



tbcst <- tbcst %>% gather(
  tb, tbcst, exp_dstb, exp_mdrtb,
  c.opd_dstb.m, c.opd_mdrtb.m, c.ipd_dstb.m, c.ipd_mdrtb.m
)
tbcst <- setDT(tbcst)
tbcst <- tbcst[!(tbcst == 0)]

tbcst[, log.gdp := log(gdp)]
tbcst[, log.tbcst := log(tbcst)]
plot.labels <- c(
  exp_dstb = "Drug-susceptible TB (DS TB), NTP costs",
  exp_mdrtb = "Multidrug-resistant TB (MDR TB), NTP costs",
  dstb.opd = "Drug-susceptible TB (DS TB), OPD costs",
  mdrtb.opd = "Multidrug-resistant TB (MDR TB), OPD costs",
  dstb.ipd = "Drug-susceptible TB (DS TB), IPD costs",
  mdrtb.ipd = "Multidrug-resistant TB (MDR TB), IPD costs"
)
...
reg.data <- data.table(
  x = exp(log.log.lr$model$log.gdp),
...
plot <- ggplot(reg.data, aes(x = gdp, y = tbcst)) +
  geom_jitter() +
  geom_line(
    data = reg.data, aes(x = gdp, y = fitted.cst),
    color = viridis::viridis(1, end = 0), size = 0.7
  ) +
  geom_ribbon(data = reg.data, aes(ymin = lwr, ymax = upr), alpha = .15) +
  scale_x_continuous(
    trans = "log2", labels = comma,
    breaks = c(100, 500, 1000, 5000, 10000, 20000, 30000)
  ) +
  scale_y_continuous(
    trans = "log2", labels = comma,
    breaks = c(
  ...
  facet_wrap(~tb,
    scales = "free_y",
    labeller = labeller(tb = plot.labels), ncol = 2
  ) +
  ggthemes::theme_base() +
  labs(
    x = "GDP per capita (2020 US$)",
    y = "Cost per patient treated (2020 US$)"
  )

ggsave(
  plot = plot,
  filename = here("plots", "TB costs regression simple.png"),
  w = 10, h = 5, dpi = 600
)
ggsave(
  plot = plot,
  filename = here("plots", "TB costs regression simple.pdf"),
  w = 10, h = 5, dpi = 600
)
...
reg.ggplot <- ggplot(
  ...
  geom_point(aes(size = e_inc_ds_num / pi, fill = g_whoregion),
    alpha = 0.75, shape = 21
  ) +
  geom_line(
    data = reg.data, aes(x = gdp, y = fitted.cst),
  ...
  ggrepel::geom_text_repel(
    data = reg.data %>% filter(e_inc_ds_num > 100), aes(label = country),
  ...
  scale_x_continuous(
    trans = "log2", labels = comma,
    breaks = c(0, 500, 1000, 5000, 10000, 20000, 30000, 50000)
  ) +
  scale_y_continuous(
    trans = "log2", labels = comma,
    breaks = c(
...
reg.plot <- reg.ggplot + geom_point(
  data = legend_bubbles,
  ...
  geom_text(
    data = legend_bubbles, size = 3,
    aes(x = 400, y = -15000 + 1 * radius / 1, label = label)
  ) +
  annotate("text",
    x = 300, y = 35000,
    label = "TB caseload (notified TB cases)"
  )
...
ggsave(
  plot = reg.plot,
  filename = here("plots", "TB costs regression bubble.png"),
  w = 10, h = 5, dpi = 600
)
ggsave(
  plot = reg.plot,
  filename = here("plots", "TB costs regression bubble.pdf"),
  w = 10, h = 5, dpi = 600
)
