...
HHMDR_costnames <- c(
  "c_hh_visit", "c_hiv_test", "c_tst_test", "c_opd_visit", "c_cxr_exam",
...
costs <- c(
  "c_hh_visit", "c_hiv_test", "c_tst_test", "c_opd_visit", "c_cxr_exam", "c.qft.plus",
  ...
  "c_aes_FQ", "c_aes_BDQ", "c_saes_INH", "c_saes_FQ", "c_saes_BDQ", "c_dstb_tx014", "c_mdrtb_tx1.04",
  "c_mdrtb_tx2.04",
  "c_mdrtb_tx1.514", "c_mdrtb_tx2.514", "c_dstb_txO15", "c_mdrtb_txO15", "c_xdrtb_txO15", "c_tpt_O15",
  "c_monit", "c_aes",
...
description <- c(
  "Household visit", "HIV test", "Tuberculin skin test", "Outpatient visit", "Chest X-ray",
  "QuantiFERON-TB Gold test",
  "Expectoration sputum collection", "Induced sputum collection", "Shared sample transportion",
  "Exclusive sample transportion",
  ...
  "Delamanid-based TPT (0-4 years)", "Delamanid-based TPT (5-14 years)", "TPT follow-up",
  "Isoniazid-based TPT monitoring",
  "Fluoroquinolone-based TPT monitoring", "Bedaquiline-based TPT monitoring",
  "Isoniazid-based TPT adverse events treatment",
  "Fluoroquinolone-based TPT adverse events treatment", "Bedaquiline-based TPT adverse events treatment",
  "Isoniazid-based TPT serious adverse events treatment",
  "Fluoroquinolone-based TPT serious adverse events treatment",
...
hhcm_costs <- hhcm_costs %>%
  group_by(country) %>%
  mutate(resource = description)
...
hhcm_costs$income <- ifelse(is.na(hhcm_costs$income) & hhcm_costs$gdp.pp > 12696,
...
visits.lo <- -0.0272
visits.hi <- -0.00119
vsts.per.provider <- -0.0412
vsts.per.provider.lo <- -0.0578
vsts.per.provider.hi <- -0.0246
urban.opd <- 0.352
urban.opd.lo <- 0.268
urban.opd.hi <- 0.435
public.opd <- -0.290
...
public.opd.hi <- -0.249
private.opd <- 0.0532
private.opd.lo <- 0.00479
private.opd.hi <- 0.102
level.2 <- 0.208
level.2.lo <- 0.144
level.2.hi <- 0.271
level.3 <- 0.304
...
level.4.hi <- 0.417
Colombia.opd <- 0.628
Colombia.opd.lo <- 0.542
Colombia.opd.hi <- 0.713
Brazil.opd <- -1.563
Brazil.opd.lo <- -1.656
Brazil.opd.hi <- -1.470
Brazil.level.3 <- -0.245
Brazil.level.3.lo <- -0.337
Brazil.level.3.hi <- -0.153
constant.opd <- -4.534
constant.opd.lo <- -4.797
constant.opd.hi <- -4.271
...
occupancy <- -0.0201
occupancy.lo <- -0.0340
occupancy.hi <- -0.00623
a.los <- -0.600
a.los.lo <- -0.649
a.los.hi <- -0.550
admissions <- 0.0252
admissions.lo <- 0.00471
admissions.hi <- 0.0457
district <- -0.204
district.lo <- -0.275
district.hi <- -0.132
teaching <- 0.257
...
public.ipd.hi <- -0.107
private.ipd <- 0.110
private.ipd.lo <- 0.0710
private.ipd.hi <- 0.148
Brazil.ipd <- -1.638
Brazil.ipd.lo <- -1.694
Brazil.ipd.hi <- -1.583
constant.ipd <- -4.277
constant.ipd.lo <- -5.035
constant.ipd.hi <- -3.519

# table(tb_exp$g_whoregion,tb_exp$hosp_type_mdr)
ipd.mean <- exp(constant.ipd + gdppc.ipd * log(hhcm_costs$gdp.pp[hhcm_costs$unit_cost == "c_hh_visit"]) +
  occupancy * log(occupancy.l3) + a.los * log(a.los.l3) +
  admissions * log(admission.l3) + district + public.ipd + Brazil.ipd)

ipd.lo <- exp(constant.ipd.lo + gdppc.ipd.lo * log(hhcm_costs$gdp.pp[hhcm_costs$unit_cost == "c_hh_visit"]) +
  occupancy.lo * log(occupancy.l3) + a.los.lo * log(a.los.l3) + admissions.lo * log(admission.l3) +
  district.lo + public.ipd.lo + Brazil.ipd.lo)

ipd.hi <- exp(constant.ipd.hi + gdppc.ipd.hi * log(hhcm_costs$gdp.pp[hhcm_costs$unit_cost == "c_hh_visit"]) +
  occupancy.hi * log(occupancy.l3) + a.los.hi * log(a.los.l3) + admissions.hi * log(admission.l3) +
  district.hi + public.ipd.hi + Brazil.ipd.lo)
...
hhcm_costs$cost.sd[hhcm_costs$unit_cost == "c_hh_visit"] <-
  (
    unit_costs$`unit cost`[unit_costs$`cost parameter` == "c.contact.tracing" &
      unit_costs$setting == "Peru"] * 150 / 100 -
      unit_costs$`unit cost`[unit_costs$`cost parameter` == "c.contact.tracing" &
...
hhcm_costs$cost.sd[hhcm_costs$unit_cost == "c_hiv_test"] <-
  ifelse(hhcm_costs$income == "High income",
    (
      unit_costs$parm2[unit_costs$`cost parameter` == "c.hiv" &
        unit_costs$setting == "hic"] -
...
# visits
c.opd_dstb.m <- ifelse(is.na(hhcm_costs$c.opd_dstb.m[unique(hhcm_costs$country)]) |
  hhcm_costs$c.opd_dstb.m[unique(hhcm_costs$country)] == 0,
exp(predict(
  llr.carecst.dstb.opd,
  data.frame(gdp_log = log(hhcm_costs$gdp.pp[hhcm_costs$unit_cost == "c_dstb_tx014"]))
)),
hhcm_costs$c.opd_dstb.m[unique(hhcm_costs$country)]
)

c.opd_dstb.lo <- ifelse(is.na(hhcm_costs$c.opd_dstb.hi[unique(hhcm_costs$country)]) |
  hhcm_costs$c.opd_dstb.lo[unique(hhcm_costs$country)] == 0,
exp(predict(llr.carecst.dstb.opd,
  data.frame(gdp_log = log(hhcm_costs$gdp.pp[hhcm_costs$unit_cost == "c_dstb_tx014"])),
  interval = "confidence"
))[, 2],
...
c.opd_dstb.hi <- ifelse(is.na(hhcm_costs$c.opd_dstb.hi[unique(hhcm_costs$country)]) |
  hhcm_costs$c.opd_dstb.hi[unique(hhcm_costs$country)] == 0,
exp(predict(llr.carecst.dstb.opd,
  data.frame(gdp_log = log(hhcm_costs$gdp.pp[hhcm_costs$unit_cost == "c_dstb_tx014"])),
  interval = "confidence"
))[, 3],
...
c.ipd_dstb.m <- ifelse(is.na(hhcm_costs$c.ipd_dstb.m[unique(hhcm_costs$country)]) |
  hhcm_costs$c.ipd_dstb.m[unique(hhcm_costs$country)] == 0,
...
c.ipd_dstb.lo <- ifelse(is.na(hhcm_costs$c.ipd_dstb.lo[unique(hhcm_costs$country)]) |
  hhcm_costs$c.ipd_dstb.lo[unique(hhcm_costs$country)] == 0,
exp(predict(llr.carecst.dstb.ipd, data.frame(gdp_log = log(hhcm_costs$gdp.pp[hhcm_costs$unit_cost == "c_dstb_tx014"])),
  interval = "confidence"
))[, 2],
...
c.ipd_dstb.hi <- ifelse(is.na(hhcm_costs$c.ipd_dstb.hi[unique(hhcm_costs$country)]) |
  hhcm_costs$c.ipd_dstb.hi[unique(hhcm_costs$country)] == 0,
...
c.dstb.drugs.sd <- (unit_costs$parm2[unit_costs$`cost parameter` == "c.dstb.drugs" &
  unit_costs$setting == "World"] -
  unit_costs$parm1[unit_costs$`cost parameter` == "c.dstb.drugs" & unit_costs$setting == "World"]) / 4


hhcm_costs$cost.m[hhcm_costs$unit_cost == "c_dstb_tx014"] <- (prog_vh_dstb +
  unit_costs$`unit cost`[unit_costs$`cost parameter` == "c.dstb.drugs" & unit_costs$setting == "World"])
...
hhcm_costs$cost.m[hhcm_costs$unit_cost == "c_dstb_drugs_tx"] <- unit_costs$`unit cost`[
  unit_costs$`cost parameter` == "c.dstb.drugs" &
    unit_costs$setting == "World"
]
...
ntp_mdrtb.m <- ifelse(is.na(hhcm_costs$exp_mdrtb[unique(hhcm_costs$country)]),
  exp(predict(
    ...
    data.frame(
      log.gdp =
        log(hhcm_costs$gdp.pp[hhcm_costs$unit_cost == "c_dstb_tx014"])
    )
  )),
...
ntp_mdrtb.lo <- ifelse(is.na(hhcm_costs$exp_mdrtb[unique(hhcm_costs$country)]),
  exp(predict(llr.ntpcst.mdrtb,
    data.frame(log.gdp = log(hhcm_costs$gdp.pp[hhcm_costs$unit_cost == "c_dstb_tx014"])),
    interval = "confidence"
  ))[, 2],
...
# visits
c.opd_mdrtb.m <- ifelse(is.na(hhcm_costs$c.opd_mdrtb.m[unique(hhcm_costs$country)]) |
  hhcm_costs$c.opd_mdrtb.m[unique(hhcm_costs$country)] == 0,
exp(predict(
  llr.carecst.mdrtb.opd,
  data.frame(gdp_log = log(hhcm_costs$gdp.pp[hhcm_costs$unit_cost == "c_dstb_tx014"]))
)),
hhcm_costs$c.opd_mdrtb.m[unique(hhcm_costs$country)]
)

c.opd_mdrtb.lo <- ifelse(is.na(hhcm_costs$c.opd_mdrtb.lo[unique(hhcm_costs$country)]) |
  hhcm_costs$c.opd_mdrtb.lo[unique(hhcm_costs$country)] == 0,
exp(predict(llr.carecst.mdrtb.opd,
  data.frame(gdp_log = log(hhcm_costs$gdp.pp[hhcm_costs$unit_cost == "c_dstb_tx014"])),
  interval = "confidence"
))[, 2],
...
c.opd_mdrtb.hi <- ifelse(is.na(hhcm_costs$c.opd_mdrtb.hi[unique(hhcm_costs$country)]) |
  hhcm_costs$c.opd_mdrtb.hi[unique(hhcm_costs$country)] == 0,
exp(predict(llr.carecst.mdrtb.opd,
  data.frame(gdp_log = log(hhcm_costs$gdp.pp[hhcm_costs$unit_cost == "c_dstb_tx014"])),
  interval = "confidence"
))[, 3],
...
c.ipd_mdrtb.m <- ifelse(is.na(hhcm_costs$c.ipd_mdrtb.m[unique(hhcm_costs$country)]) |
  hhcm_costs$c.ipd_mdrtb.m[unique(hhcm_costs$country)] == 0,
exp(predict(
  llr.carecst.dstb.ipd,
  data.frame(gdp_log = log(hhcm_costs$gdp.pp[hhcm_costs$unit_cost == "c_dstb_tx014"]))
)),
hhcm_costs$c.ipd_mdrtb.m[unique(hhcm_costs$country)]
)

c.ipd_mdrtb.lo <- ifelse(is.na(hhcm_costs$c.ipd_mdrtb.lo[unique(hhcm_costs$country)]) |
  hhcm_costs$c.ipd_mdrtb.lo[unique(hhcm_costs$country)] == 0,
exp(predict(llr.carecst.dstb.ipd,
  data.frame(gdp_log = log(hhcm_costs$gdp.pp[hhcm_costs$unit_cost == "c_dstb_tx014"])),
  interval = "confidence"
))[, 2],
...
c.ipd_mdrtb.hi <- ifelse(is.na(hhcm_costs$c.ipd_mdrtb.hi[unique(hhcm_costs$country)]) |
  hhcm_costs$c.ipd_mdrtb.hi[unique(hhcm_costs$country)] == 0,
exp(predict(llr.carecst.dstb.ipd,
  data.frame(gdp_log = log(hhcm_costs$gdp.pp[hhcm_costs$unit_cost == "c_dstb_tx014"])),
  interval = "confidence"
))[, 3],
...
cpp.dstb <- ifelse(is.na(hhcm_costs$cpp.dstb[unique(hhcm_costs$country)]) |
  hhcm_costs$cpp.dstb[unique(hhcm_costs$country)] == 0,
exp(predict(
  llr.carecst.dstb.drugs,
  data.frame(gdp_log = log(hhcm_costs$gdp.pp[hhcm_costs$unit_cost == "c_dstb_tx014"]))
)),
hhcm_costs$cpp.dstb[unique(hhcm_costs$country)]
)

cpp.mdrtb <- ifelse(is.na(hhcm_costs$cpp.mdrtb[unique(hhcm_costs$country)]) |
  hhcm_costs$cpp.mdrtb[unique(hhcm_costs$country)] == 0,
exp(predict(
  llr.carecst.mdrtb.drugs,
  data.frame(gdp_log = log(hhcm_costs$gdp.pp[hhcm_costs$unit_cost == "c_dstb_tx014"]))
)),
hhcm_costs$cpp.mdrtb[unique(hhcm_costs$country)]
)

cpp.xdrtb <- ifelse(is.na(hhcm_costs$cpp.xdrtb[unique(hhcm_costs$country)]) |
  hhcm_costs$cpp.xdrtb[unique(hhcm_costs$country)] == 0,
exp(predict(
  llr.carecst.xdr.drugs,
  data.frame(gdp_log = log(hhcm_costs$gdp.pp[hhcm_costs$unit_cost == "c_dstb_tx014"]))
)),
hhcm_costs$cpp.xdrtb[unique(hhcm_costs$country)]
)

cpp.tpt <- ifelse(is.na(hhcm_costs$cpp.tpt[unique(hhcm_costs$country)]) |
  hhcm_costs$cpp.tpt[unique(hhcm_costs$country)] == 0,
exp(predict(
  llr.carecst.tpt.drugs,
  data.frame(gdp_log = log(hhcm_costs$gdp.pp[hhcm_costs$unit_cost == "c_dstb_tx014"]))
)),
...
c.mdrtb.drugs1.04 <- tx.dur1 * (colSums(unit_costs %>%
  filter(unit_costs$`cost parameter` %in%
    c("c.cs1", "c.cfz1", "c.lin1")) %>%
  select(`unit cost`)) + c.bdq1)


c.mdrtb.drugs1.04.sd <- sqrt(c.bdq1.sd^2 + (tx.dur1 * (colSums(unit_costs %>%
  filter(unit_costs$`cost parameter` %in%
    c("c.cs1", "c.cfz1", "c.lin1")) %>%
  select(parm2)) - colSums(unit_costs %>% filter(unit_costs$`cost parameter` %in%
  c("c.cs1", "c.cfz1", "c.lin1")) %>%
  select(parm1))) / 4)^2)
...
c.mdrtb.drugs2.04 <- tx.dur1 * (colSums(unit_costs %>%
  filter(unit_costs$`cost parameter` %in% c("c.lvx1", "c.cfz1", "c.lin1")) %>%
  select(`unit cost`)) + c.bdq1)

c.mdrtb.drugs2.04.sd <- sqrt(c.bdq1.sd^2 + (tx.dur1 * (colSums(unit_costs %>%
  filter(unit_costs$`cost parameter` %in% c("c.lvx1", "c.cfz1", "c.lin1")) %>% select(parm2)) -
  colSums(unit_costs %>%
    filter(unit_costs$`cost parameter` %in% c("c.lvx1", "c.cfz1", "c.lin1")) %>%
    select(parm1))) / 4)^2)
...
c.mdrtb.drugs1.514 <- tx.dur2 * (colSums(unit_costs %>%
  filter(unit_costs$`cost parameter` %in% c("c.cs2", "c.cfz2", "c.lin2")) %>%
  select(`unit cost`)) + c.bdq2)

c.mdrtb.drugs1.514.sd <- sqrt(c.bdq2.sd^2 + (tx.dur2 * (colSums(unit_costs %>%
  filter(unit_costs$`cost parameter` %in% c("c.cs2", "c.cfz2", "c.lin2")) %>%
  select(parm2)) -
  colSums(unit_costs %>%
    filter(unit_costs$`cost parameter` %in% c("c.cs2", "c.cfz2", "c.lin2")) %>%
...
c.mdrtb.drugs2.514.sd <- sqrt(c.bdq2.sd^2 + (tx.dur2 * (colSums(unit_costs %>%
  filter(unit_costs$`cost parameter` %in% c("c.lvx2", "c.cfz2", "c.lin2")) %>%
  select(parm2)) -
  colSums(unit_costs %>%
    filter(unit_costs$`cost parameter` %in% c("c.lvx2", "c.cfz2", "c.lin2")) %>%
    select(parm1))) / 4)^2)
...
TBM_unitcosts <- hhcm_costs |>
  ...
  transmute(
    who_region = who.region, country, Iso3 = iso3,
...
summary_costs %>%
  ...
  mutate(unit_cost = factor(unit_cost,
    ...
    labels = c(
      "Household visit", "HIV test", "TST test",
  ...
  geom_errorbar(aes(ymin = ifelse(cost.m - cost.sd < 0, 0.5, cost.m - cost.sd), ymax = cost.m + cost.sd),
    width = .2,
...
resources_complete %>% ggplot(aes(hospd_dstb_prct)) +
  geom_density()
...
ggplot(resources_complete %>% mutate(variable = "cnts"), aes(x = variable, y = hospd_dstb_prct)) +
  ...
  theme_minimal() +
  labs(
    title = "Percentage of admitted DSTB across 211 countries",
    x = "",
    y = "Percentage"
  )
...
ggplot(resources_complete %>% mutate(variable = "cnts"), aes(x = variable, y = hospd_dstb_dur)) +
  ...
  theme_minimal() +
  labs(
    title = "LoHS for admitted DSTB across 211 countries",
    x = "",
    y = "Days"
  )
