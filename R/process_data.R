...
notif <- read_csv("data/TB_notifications_2025-05-21.csv") %>%
  ...
  select(
    country, iso3, year,
    newrel_f04, newrel_f014,
    newrel_m04, newrel_m014
  ) %>% # filter(year%in%c(2023:2024))%>%
  mutate(
    notif04 = newrel_f04 + newrel_m04,
    notif014 = newrel_f014 + newrel_m014
  ) %>%
  select(Iso3 = iso3, notif04, notif014) %>%
  pivot_longer(
    cols = c("notif04", "notif014"),
    names_to = "Age", values_to = "notif"
  ) %>%
...
uc_vax_delv <- uc_vax_delv %>%
  ...
  rename_with(~ c(
    "Country", "uc_tot_vax_delv_ave",
    "uc_tot_vax_delv_med", "uc_tot_vax_delv_lo", "uc_tot_vax_delv_hi",
...
uc_vax_delv$Iso3 <- countrycode(uc_vax_delv$Country,
  origin = "country.name",
  destination = "iso3c"
)
