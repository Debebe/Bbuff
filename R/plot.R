...
bcgplot <- function(BCG,Isos){
  BCG%>%filter(Year==2023, !is.na(coverage), Iso3%in%Isos) %>%
    ggplot(aes(cat, coverage, fill =cat ))+geom_bar(stat="identity")+ 
    theme(axis.text.x = element_text(angle = 90), legend.title = element_blank())+
    geom_text(aes(x=cat, label=coverage), vjust=1.6, 
              color="black",size=3.0)+
    facet_wrap(~Iso3)+ xlab("") +ylab("BCG coverage")
