...
tail(names(r.gdp), 1)
indicators <- c(
  "Country Name", "Country Code", "2016", "2017", "2018", "2019", "2020", "2021", "2022", "2023",
  "2024"
)
...
if (!file.exists(here::here("data/cost/indata/exch.Rdata"))) {
  ...
  exch <- setDT(readxl::read_excel(tf, sheet = "Data", skip = 3))
...
if (!file.exists(here::here("data/cost/indata/exch_ppp.Rdata"))) {
  ...
  exch_ppp <- setDT(readxl::read_excel(tf, sheet = "Data", skip = 3))
...
if (!file.exists(here::here("data/cost/indata/ppp.Rdata"))) {
  ...
  ppp <- setDT(readxl::read_excel(tf, sheet = "Data", skip = 3))
  # ppp <- setDT(read_csv(here('data','ppp.csv')))
...
if (!file.exists(here::here("data/cost/indata/defl.Rdata"))) {
  ...
  defl <- setDT(readxl::read_excel(tf, sheet = "Data", skip = 3))
  # ppp <- setDT(read_csv(here('data','ppp.csv')))
...
gdp <- r.gdp %>%
  select(c("Country Name", "Country Code", year, r.gdp)) %>%
  left_join(gdp, by = c("Country Name", "Country Code", "year"))
gdp <- exch %>%
  select(c("Country Name", "Country Code", year, exch)) %>%
  left_join(gdp, by = c("Country Name", "Country Code", "year"))
gdp <- ppp %>%
  select(c("Country Name", "Country Code", year, ppp)) %>%
  left_join(gdp, by = c("Country Name", "Country Code", "year"))
gdp <- exch_ppp %>%
  select(c("Country Name", "Country Code", year, exch_ppp)) %>%
  left_join(gdp, by = c("Country Name", "Country Code", "year"))
...
tb_budget[, bdstb := rowSums(.SD), .SDcols = c(
  "budget_fld", "budget_lab", "budget_mdrmgt",
...
expend <- c(
  "hcfvisit_dstb", "hcfvisit_mdr", "hosp_type_mdr", "hospd_dstb_dur",
  "hospd_dstb_prct", "hospd_mdr_dur", "hospd_mdr_prct",
  "exp_fld", "exp_lab", "exp_mdrmgt", "exp_prog",
  "exp_sld", "exp_staff", "exp_orsrvy", "exp_oth"
)
...
tb_exp <- df_exp[, .(
  country, iso2, iso3, iso_numeric, g_whoregion, year, hcfvisit_dstb, hcfvisit_mdr,
...
income_group <- income_group %>%
  ...
  mutate(`Country Name` = ifelse(`Country Name` == "Venezuela, RB",
...
# gdp$gdp <- gdp$'2019'
tb_exp <- gdp %>%
  select(iso3, r.gdp, gdp, ppp, exch_ppp) %>%
  left_join(tb_exp, by = "iso3")
tb_exp <- df %>%
  filter(year == analysis_year) %>%
  select(iso3, e_inc_num) %>%
  left_join(tb_exp, by = "iso3")
tb_exp <- df_mdr %>%
  filter(year == analysis_year) %>%
  select(iso3, e_inc_rr_num) %>%
  left_join(tb_exp, by = "iso3")

# search_data_dict(var = c("e_inc_num", "e_inc_rr_num"))
tb_exp[, e_inc_ds_num := ifelse(is.na(e_inc_num) & is.na(e_inc_rr_num), NA_real_,
  rowSums(.SD, na.rm = T)
), .SDcols = c("e_inc_num", "e_inc_rr_num")]
...
resource.use <- c(
  "hcfvisit_dstb", "hospd_dstb_prct", "hospd_dstb_dur", "hcfvisit_mdr",
  "hospd_mdr_prct", "hospd_mdr_dur", "hosp_type_mdr"
)
...
resources_complete[, c_opd.m := exp(
  constant.opd + gdppc.opd * log(gdp) + visits * log(opd.vsts.l1) +
    vsts.per.provider * log(vsts.per.provider.fl1) + public.opd
)]
resources_complete[, c_opd.lo := exp(
  constant.opd.lo + gdppc.opd.lo * log(gdp) +
    visits.lo * log(opd.vsts.l1) + vsts.per.provider.lo * log(vsts.per.provider.fl1) +
    public.opd.lo
)]
resources_complete[, c_opd.hi := exp(
  constant.opd.hi + gdppc.opd.hi * log(gdp) +
    visits.hi * log(opd.vsts.l1) + vsts.per.provider.hi * log(vsts.per.provider.fl1) +
    public.opd.hi
)]
...
# IPD stay unit costs
resources_complete[, c_ipd.m := exp(
  constant.ipd + gdppc.ipd * log(gdp) +
    occupancy * log(occupancy.l3) + a.los * log(a.los.l3) +
    admissions * log(admission.l3) + district + public.ipd + Brazil.ipd
)]

resources_complete[, c_ipd.lo := exp(
  constant.ipd.lo + gdppc.ipd.lo * log(gdp) + occupancy.lo * log(occupancy.l3) +
    a.los.lo * log(a.los.l3) + admissions.lo * log(admission.l3) + district.lo + public.ipd.lo + Brazil.ipd.lo
)]

resources_complete[, c_ipd.hi := exp(
  constant.ipd.hi + gdppc.ipd.hi * log(gdp) + occupancy.hi * log(occupancy.l3) +
    a.los.hi * log(a.los.l3) + admissions.hi * log(admission.l3) + district.hi + public.ipd.hi + Brazil.ipd.hi
)]
...
resources_complete %>%
  ...
  pivot_longer(
    cols = c(cpp_dstb, cpp_mdr, cpp_xdr, cpp_tpt),
...
tb_exp |>
  ...
  select(
    iso3, country, exp_lab, exp_staff, exp_prog, exp_orsrvy, exp_patsup,
...
resources_complete <- resources_complete %>%
  mutate(across(
    c(cpp_dstb, cpp_mdr, cpp_xdr, cpp_tpt),
...
exp_complete[, exp_mdrmgt := ifelse(country == "Russian Federation",
  (exp_mdrmgt + exp_lab * 71 / 100 + exp_staff * 71 / 100), exp_mdrmgt
)]


# DSTB & MDRTB programme costs
exp_complete[, exp_dstb := rowSums(.SD),
  .SDcol = c("exp_lab", "exp_prog", "exp_staff", "exp_orsrvy", "exp_patsup")
]
...
prog_costs <- exp_complete[, .(
  country, iso3, g_whoregion, gdp, tx_dstb, tx_mdr,
  tx_xdr, exp_dstb, exp_mdrtb, e_inc_ds_num, e_inc_rr_num
)]
...
care_costs <- resources_complete[, .(
  country, iso3, g_whoregion, gdp,
  tx_dstb, tx_mdr, tx_xdr,
...
ntpcst <- ntpcst[!(exp_dstb == 0)]
ntpcst <- ntpcst[!(exp_mdrtb == 0)]
ntpcst <- ntpcst %>% gather(tb, ntpexp, exp_dstb, exp_mdrtb)
ntpcst <- setDT(ntpcst)
ntpcst[, log.gdp := log(gdp)]
ntpcst[, log.ntpcst := log(ntpexp)]
...
ntp.data <- data.table(
  x = exp(llr.ntpcst$model$log.gdp),
...
plot <- ggplot(ntp.data, aes(x = gdp, y = ntpexp)) +
  ...
  #               "dashed")  +
  geom_line(
    data = ntp.data, aes(x = gdp, y = cstntp),
...
care <- c(
  "gdp", "c.opd_dstb.m", "c.opd_mdrtb.m",
  "c.ipd_dstb.m", "c.ipd_mdrtb.m",
  "cpp_dstb", "cpp_mdr", "cpp_xdr", "cpp_tpt"
)
...
tb <- c(
  "dstb.opd", "mdrtb.opd", "dstb.ipd", "mdrtb.ipd",
  "cpp_dstb", "cpp_mdr", "cpp_xdr", "cpp_tpt"
)
plot.labels <- c(
  dstb.opd = "Drug-susceptible TB (DS TB), OPD",
...
tbcst.care <- tbcst.care %>%
  gather(
    tb, log.cost, c.opd_dstb.m_log, c.opd_mdrtb.m_log,
...
# Ploting the log-log regression model results in the original scales
care.data <- data.table(
  x = exp(llr.carecst$model$gdp_log),
...
plot <- ggplot(care.data, aes(x = gdp, y = exp(log.cost))) +
  ...
  #               "dashed")  +
  geom_line(
    data = care.data, aes(x = gdp, y = care.cost),
    color = viridis::viridis(1, end = 0), size = 0.7
  ) +
